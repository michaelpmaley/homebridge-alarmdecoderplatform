...
AlarmDecoderPlatform.prototype.configureAccessory = function(accessory) {
   ...
   // contact sensor: rebind characteristics to local functions and sync state
   ...
   // motion sensor: rebind characteristics to local functions and sync state
...
AlarmDecoderPlatform.prototype.addZones = function() {
  this.log("AddZones");
   ...
   zones.forEach(function(zone) {
      ...
      // contact sensor: add Service.ContactSensor and bind to local functions
      if (accessory.context.type === "contact") {
         var service = accessory.addService(Service.ContactSensor);
         service.getCharacteristic(Characteristic.ContactSensorState)
           .on('get', this.getZoneCurrentState.bind(this, accessory.context.id));

      // motion sensor: add Service.MotionSensor and bind to local functions
      } else if (accessory.context.type === "motion") {
         var service = accessory.addService(Service.MotionSensor);
         service.getCharacteristic(Characteristic.MotionDetected)
            .on('get', this.getZoneCurrentState.bind(this, accessory.context.id));
...
/**
 * Homekit request to get current zone state in order for it to set the zone accessory state.
 * @param {object} callback - The Homebridge callback function (error, state).
 */
AlarmDecoderPlatform.prototype.getZoneCurrentState = function(id, callback) {
   this.log("GetZoneCurrentState: %s", id);

   this.getADZoneState(id, callback);
}


/**
 * Helper function to sync zone accessory state with alarmdecoder zone state
*/
AlarmDecoderPlatform.prototype.syncZoneState = function(id, callback) {
   ...
   this.getADZoneState(id, function(error, state) {
      ...
      this.setHKZoneState(id, state, callback);
   }.bind(this));
}
...
/**
 * Helper function to get the alarmdecoder zone state.
 * @param {object} callback - The callback function (error, state).
 */
AlarmDecoderPlatform.prototype.getADZoneState = function(id, callback) {
   ...
   request( { method: method, url: url, headers: {'Authorization': this.config.key, 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: body }, function(error, response, body) {
      ...
      // {"last_message_received": "[10000001100000003A--],008,[f722000f1008001c28020000000000],\" DISARMED CHIME   Ready to Arm  \"", "panel_alarming": false, "panel_armed": false, "panel_armed_stay": false, "panel_bypassed": {}, "panel_fire_detected": false, "panel_on_battery": false, "panel_panicked": false, "panel_powered": true, "panel_relay_status": [], "panel_type": "ADEMCO", "panel_zones_faulted": [] }
      var stateObj = JSON.parse(body);
      var zonesFaulted = stateObj.panel_zones_faulted;
      var zoneId = parseInt(accessory.context.id, 10);
      var zoneState = (zonesFaulted != null && zonesFaulted.indexOf(zoneId) !== -1) ? true : false;
      //this.log.debug("have %s, %s, %s", zoneId, zonesFaulted, zoneState);

      // contact sensor:
      if (accessory.context.type === "contact") {
         var cstate = zoneState ? Characteristic.ContactSensorState.CONTACT_NOT_DETECTED : Characteristic.ContactSensorState.CONTACT_DETECTED;
         this.log("current alarmdecoder zone %s state = %s", accessory.context.id, cstate);
